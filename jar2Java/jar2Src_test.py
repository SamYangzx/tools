    ...
    for root, _, files in os.walk(java_dir):
        for file in files:
            if file.endswith(".java"):
                ...
                # 解析 IInterface 接口并去除内部类
                if "extends android.os.IInterface" in java_code:
...
def convert_to_aidl(java_code):
    ...
    # 只保留接口定义，去掉继承信息
    aidl_code = re.sub(r'public interface (\w+) extends android\.os\.IInterface', r'interface \1', aidl_code)
    # 移除 RemoteException
    aidl_code = re.sub(r'public ([\w<>]+) (\w+)\((.*?)\) throws RemoteException;', r'\1 \2(\3);', aidl_code)
    # 删除可能遗留的空白行
    aidl_code = re.sub(r'\n\s*\n', '\n', aidl_code)
    return aidl_code.strip()
...
if __name__ == "__main__":
    jar_path = "your_file.jar"  # 替换为你的 JAR 文件路径
